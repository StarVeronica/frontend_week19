...
function makeTwo() {
	fetch('https://emojihub.yurace.pro/api/random/group/face-positive')
		...
		.then((data) => {
			...
			newItem.innerHTML = htmlCode;
			document.getElementById('result2').append(newItem);
		})
	  	.catch((error) => {
			console.error('Ошибка:', error);
	  	});
...
function makeTen() {
	...
	.then((data) => {
		...
		newItem.src = message;
		document.getElementById('result10').append(newItem);
	})
	  .catch((error) => {
		console.error('Ошибка:', error);
	  });
...
function makeEleven() {
	fetch('https://api.ipify.org?format=json')
		...
		.then((data) => {
			const ip = data.ip;
			const newItem = document.createElement('p');
			newItem.textContent = ip;
			document.getElementById('result11').append(newItem);
		})
		.catch((error) => {
			console.error('Ошибка:', error);
		});
...
function makeTwelve() {
	...
	const link = `http://ip-api.com/json/${input}`;
	fetch(link)
	.then((response) => response.json())
	.then((data) => {
		const newItem = document.createElement('p');
		newItem.textContent = JSON.stringify(data);
		document.getElementById('result12').append(newItem);
	})
	.catch((error) => {
		console.error('Ошибка:', error);
	});
...
function makeFourteen() {
	fetch('https://official-joke-api.appspot.com/random_joke')
		...
		.then((data) => {
			...
			newItem.innerHTML = data.setup;
			document.getElementById('result14').append(newItem);
			newItem = document.createElement('p');
			newItem.innerHTML = data.punchline;
			document.getElementById('result14').append(newItem);
		})
